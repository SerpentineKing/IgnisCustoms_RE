...
function s.initial_effect(c)
	...
	e1:SetCategory(CATEGORY_COIN+CATEGORY_DESTROY+CATEGORY_DAMAGE+CATEGORY_SPECIAL_SUMMON)
...
function s.e1tgt(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		and Duel.IsExistingMatchingCard(s.e1fil,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,nil,e,tp)
	end
	...
	if Duel.CallCoin(tp) then
		...
		if Duel.Destroy(g,REASON_EFFECT)>0 then
			...
			if Duel.Damage(1-tp,sum/2,REASON_EFFECT) then
				...
				local g=Duel.SelectMatchingCard(tp,s.e1fil,tp,LOCATION_HAND+LOCATION_DECK+LOCATION_GRAVE,0,1,1,nil,e,tp)
...
function s.e2con(e,tp,eg,ep,ev,re,r)
...
function s.e2tgt(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(s.e2fil,tp,LOCATION_GRAVE,0,1,nil,e,tp)
	end
...
function s.e2evt(e,tp)
	...
	local g=Duel.SelectMatchingCard(tp,s.e2fil,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
	if g:GetCount()>0 then
