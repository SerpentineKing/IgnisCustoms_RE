...
function s.e2evt(e,tp)
	...
	if tc and Duel.ChangePosition(tc,POS_FACEDOWN_DEFENSE)>0 then
		...
		e2b0:SetType(EFFECT_TYPE_FIELD)
		e2b0:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
		e2b0:SetLabelObject(tc)
		e2b0:SetTargetRange(0,LOCATION_MZONE)
		e2b0:SetTarget(s.e2lim)
		e2b0:SetReset(RESET_EVENT+RESETS_STANDARD+RESET_PHASE+PHASE_DRAW+RESET_OPPO_TURN,1)
		...
		e2b2:SetValue(1)
		Duel.RegisterEffect(e2b2,tp,true)

		local e2b3=e2b0:Clone()
		e2b3:SetCode(EFFECT_UNRELEASABLE_NONSUM)
		e2b3:SetValue(1)
		Duel.RegisterEffect(e2b3,tp,true)
